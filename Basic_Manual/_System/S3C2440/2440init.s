...
nand_boot_beg
	...
	bl	ReadNandID       ;返回值在r5中
	mov	r6, #0 		     ;判断芯片编号
	ldr	r0, =0xec73
	cmp	r5,	r0
	beq	%F1
	ldr	r0, =0xecda	     ;ecda即256MFlash，r6=0
	cmp	r5, r0
	beq	%F1              ;ec73和ec75是24位地址Flash 我们的是ecf1,32位地址
	mov	r6, #1           ;所以我们的Flash执行这里r6变成了1
1	
	bl	ReadNandStatus   ;状态返回值在r1
	ldr      r5,=0x56000014	
	ldr      r1,=~(1<<5)
	str      r1,[r5]
	mov	r8, #0
...
2	
	...
	;ands	r0, r8, #0x1f  ;循环检查坏块，r8
	;bne		%F3
	mov		r0, r8

	bl		CheckBadBlk    ;检查坏块函数，块地址是r0，返回值在r0,r0=0表示好块

	cmp		r0, #0
	addne	r8, r8, #64    ;不是0按坏块处理，地址+64
	bne		%F4			   ;坏块直接进入循环判断
3	
	mov	r0,r8
	mov	r1, r9
	bl	ReadNandPage	   ;读页面，地址是r0
...
CheckBadBlk
	...
	;bic      r0,r0,#0x1f	;addr &= ~0x1f;//地址按1自增，不用and
	ldr      r1,[r5,#4]		;NFChipEn()
	bic      r1,r1,#2
	str      r1,[r5,#4]

	mov      r1,#0x00		;WrNFCmd(0x00)
	strb     r1,[r5,#8]
	mov      r1,#0;6		;1->0
	strb     r1,[r5,#0xc]	;WrNFAddr(0)
	mov      r1,#8
	strb     r1,[r5,#0xc]	;WrNFAddr(8)
	mov      r1,r0			;WrNFAddr(addr)
	strb     r1,[r5,#0xc]		
	mov    	 r0,r0,lsr #8	;WrNFAddr(addr>>8)
	strb     r0,[r5,#0xc]
	mov		 r0,#1
	cmp		 r0,r6          
	beq	     %F1
...
;	bl		WaitNandBusy	;WaitNFBusy()
	...
	mov	r0, #100			;for(i=0;i<100;i++);
1
	subs	r0, r0, #1
	bne	%B1
2
	ldr	r0, [r5, #0x20]     ;r0=NFSTAT
	tst	r0, #1				;if(r0==1)
	beq	%B2					;	goto 2	
...
Uart0_Putch
	...
	ldr      r1,=0x50000010		;WrNFCmd(READCMD2)
1
	ldr      r1,[r1]
	and      r1,r1,#0x02
	cmp		 r1,#0
