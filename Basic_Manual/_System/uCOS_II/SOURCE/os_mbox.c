...
//此函数为OSMboxPost的剪裁
//它只向任务发出信息，而不会将信息存在事件中
#if OS_MBOX_POST_EN > 0
INT8U  OSMboxPost1 (OS_EVENT *pevent, void *msg)
{
#if OS_CRITICAL_METHOD == 3                           /* Allocate storage for CPU status register      */
    OS_CPU_SR  cpu_sr = 0;
#endif
...
//此函数为OSMboxPost的剪裁
//它函数只将信息存在事件中，会代替事件中原来的信息，接收此信息必须用OSMboxAccept
#if OS_MBOX_POST_EN > 0
INT8U  OSMboxPost2 (OS_EVENT *pevent, void *msg)
{
#if OS_CRITICAL_METHOD == 3                           /* Allocate storage for CPU status register      */
    OS_CPU_SR  cpu_sr = 0;
#endif
